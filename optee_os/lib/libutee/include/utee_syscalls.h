...
int utee_vfs_open(const char *__path, int __oflag, int arg1);

int utee_vfs_close(int fd);

int utee_vfs_access(const char *__name, int __type);

char *utee_vfs_getcwd(char *__buf, size_t __size);

int utee_vfs_stat(const char *__file_name, struct my_stat *__buf);

int utee_vfs_write(int __fd, const void *buf, size_t n);

int utee_vfs_mkdir(const char *__path, __mode_t mode);

int utee_vfs_rmdir(const char *__path);

int utee_vfs_unlink(const char *__name, int *errno);

int utee_vfs_ftruncate(int __fd, off_t __size);

ssize_t utee_vfs_read(int fd, void *buf, size_t nByte);

int utee_vfs_fchmod(int __fd, mode_t __mode);

int utee_vfs_fchown(int __fd, uid_t __owner, gid_t __group);

uid_t utee_vfs_geteuid(void);

pid_t utee_vfs_getpid(void);

char *utee_vfs_getenv(const char *__name);

off_t utee_vfs_lseek(int __fd, off_t __offset, int __whence);

int utee_vfs_fsync(int __fd);

unsigned int utee_vfs_sleep(unsigned int __seconds);

ssize_t utee_vfs_pread(int __fd, void *__buf, size_t __nByte, off_t __offset);

ssize_t utee_vfs_pwrite(int __fd, const void *buf, size_t n, off_t __offset);

int utee_vfs_truncate(const char *__path, off_t __size);

int utee_vfs_gettimeofday(struct timeval *tv, struct timezone *tz);

int utee_vfs_fstat(int __fd, struct my_stat *__buf);

int utee_vfs_fcntl_void(int __fd, int __cmd);

int utee_vfs_fcntl_int(int __fd, int __cmd, int pa);

int utee_vfs_fcntl_flock(int __fd, int __cmd, struct my_flock *pFlock);

int utee_vfs_utimes(const char *filename, const struct timeval times[2]);

time_t utee_vfs_time(time_t *t);

void utee_vfs_usleep(unsigned long usec);

ssize_t utee_vfs_filesize(int fd);
